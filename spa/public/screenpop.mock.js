// Mock CRM adapter for proof-of-concept demos (duplicated for SPA)
(function(){
  const patients = [
  ...
  const byPhone = new Map([
    ['+15551234567', patients.find(p=>p.id==='p1')],
    ['+15554567890', patients.find(p=>p.id==='p2')],
    ['+15557771234', patients.find(p=>p.id==='p3')],
  ...
  // Optional demo driver (kept minimal). SPA scenarios will override when used.
  document.addEventListener('DOMContentLoaded', async () => {
    const { phone, demo } = getQuery();
    if (!phone && !demo) return; // avoid auto-driving in SPA unless requested
    ...
    if (update) { await delay(600); window.ScreenpopAPI.applyAppointment(update); }
  });
  ...
  function getQuery(){
    try { const u = new URL(window.location.href); return { phone: u.searchParams.get('phone'), demo: u.searchParams.get('demo') }; } catch { return { phone:null, demo:null }; }
  }
