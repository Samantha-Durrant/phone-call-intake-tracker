...
const SIMULATION_PATIENTS = [
    ...
    // Test Case 5: Existing Patient with CANCELLED Appointment
    {
        id: '4',
        firstName: 'Lisa',
        lastName: 'Anderson',
        ...
        crmFlags: ['Payment Issues', 'Contact Required'],
        isExistingPatient: true,
        hasAppointment: true,
        appointmentType: 'cancelled',
        scenario: 'existing-cancelled-appointment'
    },
    
    // Test Case 6: Existing Patient with RESCHEDULED Appointment
    {
        id: '5',
        firstName: 'James',
        lastName: 'Rodriguez',
        ...
        crmFlags: ['No-Show History', 'Confirmation Required'],
        isExistingPatient: true,
        hasAppointment: true,
        appointmentType: 'rescheduled',
        scenario: 'existing-rescheduled-appointment'
    },
    
    // Test Case 7: New Patient (Unknown Number #1)
    {
        id: 'new1',
        firstName: null,
        lastName: null,
    ...
    // Test Case 8: New Patient (Unknown Number #2)
    {
        id: 'new2',
        firstName: null,
        lastName: null,
...
let appointmentStatus = {
    ...
    manualReason: null,
    notes: null,
    changeReason: null, // Why the appointment was cancelled/rescheduled
    changeReasonNotes: null,
    reasonRecorded: null
};
...
function updateScenarioInfo() {
    ...
    const scenarioDescriptions = {
        ...
        'existing-cancelled-appointment': 'Lisa Anderson - Known patient, appointment cancelled in CRM',
        'existing-rescheduled-appointment': 'James Rodriguez - Known patient, appointment rescheduled in CRM',
...
function checkAppointmentStatus() {
    ...
    setTimeout(() => {
        ...
        if (hasAppointment) {
            let typeText, iconClass, colorClass;
            
            switch(appointmentType) {
                case 'urgent':
                    typeText = 'Urgent appointment found';
                    iconClass = 'fas fa-exclamation-circle';
                    colorClass = 'text-orange';
                    break;
                case 'tentative':
                    typeText = 'Tentative appointment found';
                    iconClass = 'fas fa-calendar-alt';
                    colorClass = 'text-orange';
                    break;
                case 'cancelled':
                    typeText = 'Cancelled appointment found in CRM';
                    iconClass = 'fas fa-calendar-times';
                    colorClass = 'text-red';
                    break;
                case 'rescheduled':
                    typeText = 'Rescheduled appointment found in CRM';
                    iconClass = 'fas fa-calendar-alt';
                    colorClass = 'text-orange';
                    break;
                default:
                    typeText = 'Scheduled appointment found';
                    iconClass = 'fas fa-calendar-check';
                    colorClass = 'text-green';
            }
            
            addNewActivity(typeText, iconClass, colorClass);
        } else {
...
function updateAppointmentDisplay() {
    ...
    if (appointmentStatus.isBooked) {
        ...
        switch(appointmentStatus.appointmentType) {
            ...
            case 'cancelled':
                ...
                statusText.textContent = 'Appointment Cancelled (CRM)';
                statusIcon.className = 'fas fa-calendar-times';
                // Show reason entry for cancelled appointments
                setTimeout(() => showReasonEntry('cancelled'), 1000);
                break;
            case 'rescheduled':
                statusIndicator.className = 'status-indicator error';
                statusText.textContent = 'Appointment Rescheduled (CRM)';
                statusIcon.className = 'fas fa-calendar-alt';
                // Show reason entry for rescheduled appointments
                setTimeout(() => showReasonEntry('rescheduled'), 1000);
                break;
            default:
    ...
    // Hide reason entry for non-cancelled/rescheduled appointments
    if (appointmentStatus.appointmentType !== 'cancelled' && appointmentStatus.appointmentType !== 'rescheduled') {
        hideReasonEntry();
    }
}
...
// Appointment Reason Functions
function showReasonEntry(appointmentType) {
    const reasonEntry = document.getElementById('reasonEntry');
    const reasonHeader = document.querySelector('.reason-header span');
    
    if (appointmentType === 'cancelled') {
        reasonHeader.textContent = 'Why did the patient cancel?';
    } else if (appointmentType === 'rescheduled') {
        reasonHeader.textContent = 'Why did the patient reschedule?';
    }
    
    reasonEntry.style.display = 'block';
    
    // Reset form
    document.getElementById('appointmentReason').value = '';
    document.getElementById('reasonNotes').style.display = 'none';
    document.getElementById('appointmentReasonNotes').value = '';
    document.getElementById('reasonSaveBtn').disabled = true;
}

function hideReasonEntry() {
    document.getElementById('reasonEntry').style.display = 'none';
}

function handleAppointmentReasonChange() {
    const reason = document.getElementById('appointmentReason').value;
    const notesSection = document.getElementById('reasonNotes');
    const saveBtn = document.getElementById('reasonSaveBtn');
...
function saveAppointmentReason() {
    const reason = document.getElementById('appointmentReason').value;
    const notes = document.getElementById('appointmentReasonNotes').value;
    
    // Store the reason with the appointment status
    appointmentStatus.changeReason = reason;
    appointmentStatus.changeReasonNotes = notes;
    appointmentStatus.reasonRecorded = new Date();
    
    // Get human-readable reason text
    const reasonTexts = {
        'no-longer-needed': 'No longer needed',
        'illness-family': 'Illness/family',
        'work-school': 'Work/school',
        'insurance': 'Insurance',
        'referral': 'Referral',
        'poor-relationship': 'POOR r/s',
        'other': 'Other'
    };
    
    const reasonText = reasonTexts[reason] || reason;
    const displayText = notes && reason === 'other' ? `${reasonText}: ${notes}` : reasonText;
    
    // Update the appointment details to show the reason
    const appointmentDetails = document.getElementById('appointmentDetails');
    const currentText = appointmentDetails.textContent;
    appointmentDetails.textContent = `${currentText} • Reason: ${displayText}`;
    
    // Hide the reason entry section
    hideReasonEntry();
    
    // Log activity
    const actionType = appointmentStatus.appointmentType === 'cancelled' ? 'cancellation' : 'reschedule';
    addNewActivity(`${actionType.charAt(0).toUpperCase() + actionType.slice(1)} reason recorded: ${displayText}`, 'fas fa-clipboard-check', 'text-blue');
    showNotification('Appointment reason saved', 'success');
}

function skipAppointmentReason() {
    hideReasonEntry();
